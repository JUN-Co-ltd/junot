...
/**
 * 仕入実績CSV一覧情報Repositoryの実装クラス.
 */
@Slf4j
public class PurchaseRecordCsvCompositeRepositoryImpl implements PurchaseRecordCsvCompositeRepositoryCustom {
	 ...
	 */
	@Override
	public Page<PurchaseRecordCsvEntity> findBySearchCondition(
		...
		// 件数
		final long rst = countAllRecord(sqlWhere, parameterMap, searchCondition);
		if (rst == 0) {
			return new PageImpl<>(Collections.emptyList(), pageable, rst);
		}
		...
		final Query query = entityManager.createNativeQuery(sql.toString(), PurchaseRecordCsvEntity.class);

		// クエリにパラメータを設定
		setQueryParameters(searchCondition, query);

		@SuppressWarnings("unchecked")
		final List<PurchaseRecordCsvEntity> result = query.getResultList();
...
//           }
		...
		return new PageImpl<>(result, pageable, rst);
	}
	 ...
	 */
	@Override
	public List<PurchaseRecordCsvEntity> findPDFDetailBySearchCondition(
			final PurchaseRecordSearchConditionModel searchCondition,
			final Pageable pageable) {
...
//		sqlColumns.add("p.supplier_code AS supplier_code");
		sqlColumns.add("(CASE WHEN p.supplier_code IS NULL THEN '' ELSE p.supplier_code END) AS supplier_code");
		// PRD_0189 #10181 jfe mod end
		sqlColumns.add("SUBSTRING(mcd.item1,1,2) AS arrival_place");
...
//      sqlColumns.add("p.purchase_voucher_number AS purchase_voucher_number");
		...
		//		sqlColumns.add(" TRUNCATE(CASE WHEN p.purchase_type IN (2 , 4, 5) THEN (p.fix_arrival_count / 100) ELSE 0 END,2)  AS m_kyu");
...
//      sqlColumns.add(" TRUNCATE(CASE WHEN (purchase_type IN (2 , 4, 5)  AND p.purchase_count = 2 AND p.division_code = '20') THEN (p.fix_arrival_count / 100) ELSE 0 END,2)  AS m_kyu");
...
//      sqlColumns.add("SUM( TRUNCATE(CASE WHEN (purchase_type IN (2 , 4, 5)  AND p.purchase_count = 2 AND p.division_code = '20') THEN (p.fix_arrival_count / 100) ELSE 0 END,2))  AS m_kyu");
	    ...
	    sqlColumns.add("SUM( TRUNCATE(CASE WHEN (purchase_type IN (2 , 4, 5)  AND RIGHT(p.purchase_count,1) = 2 AND p.division_code = '20') THEN (p.fix_arrival_count / 100) ELSE 0 END,2))  AS m_kyu");
...
//		sql.append(" INNER JOIN m_codmst mcd");
//		sql.append(" ON p.arrival_place = mcd.code1");
//		sql.append(" AND mcd.deleted_at IS NULL");
//		sql.append(" AND mcd.tblid = 21");
//
...
//		}
	//PRD_0176 JFE del end
     // PRD_0189 #10181 jfe add start
      ...
      //入荷場所を物流コードから納入場所マスタの名称を取得
      sql.append(" LEFT OUTER JOIN m_codmst mcd");
      sql.append(" ON p.arrival_place = mcd.code1");
      sql.append(" AND mcd.deleted_at IS NULL");
      sql.append(" AND mcd.tblid = 21");
      // PRD_0189 #10181 jfe add end
...
//      final String arrivalShop = searchCondition.getArrivalShop().substring(4, 6); //入荷場所
		...
		if (!Objects.isNull(purchaseType)) {
			switch (purchaseType) {
			//PRD_0152 JFE  del start
//			case 0: //未選択
//				//入荷場所
//				sql.append("      AND p.arrival_place = :arrivalShop");
//				break;
			//PRD_0152 JFE  del end
			case 1: //追加仕入
...
//                  sql.append("      AND p.arrival_place = :arrivalShop");
		...
		if (!comCodeList.isEmpty()) {
			sql.append(
					"      AND substring(p.part_no,1,2) IN (select code1 from m_codmst where tblid = '02' and  item3 IN (:comCodeList))");
		}
	}
	...
	private void generateOrderByPhrase(final StringBuilder sql) {
		sql.append(" ORDER BY ");
		//PRD_0190 add JFE start
		sql.append(" p.supplier_code IS NULL ASC");
		//PRD_0190 add JFE end
