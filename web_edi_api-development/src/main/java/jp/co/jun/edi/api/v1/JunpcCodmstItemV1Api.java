...
import jp.co.jun.edi.service.JunpcCodmstItemListService;
import jp.co.jun.edi.type.MCodmstTblIdType;

/**
 * 発注生産システムのコードマスタからアイテム情報を取得するAPI.
 */
@RestController
@RequestMapping("/api/v1/junpc/codmst/items")
public class JunpcCodmstItemV1Api extends GenericJunpcCodmstV1Api {
    @Autowired
    private JunpcCodmstItemListService listService;

    /**
     * 発注生産システムのコードマスタからアイテム情報をリストで取得する.
     *
    ...
    protected MCodmstTblIdType getTblId() {
        return MCodmstTblIdType.ITEM;
    }

    @Override
    protected JunpcCodmstItemListService getListService() {
        return listService;
    }
}
