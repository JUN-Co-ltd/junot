...
@Transactional
public class FukukitaruOrderV1ApiConfilmTest {
    // モックサーバー
    @Autowired
    ...
    public void test_get_0001() throws Exception {
        ...
        test.setId(BigInteger.valueOf(1));// フクキタル発注:フクキタル発注ID
        test.setFItemId(BigInteger.valueOf(1));// フクキタル発注:フクキタル品番ID ※システム設定→DB登録時自採番
        ...
        test.setOrderCode("1_1_1");// フクキタル発注:オーダー識別コード
        test.setOrderUserId(BigInteger.valueOf(1));// フクキタル発注:発注者ユーザID ※システム設定→登録ユーザから自動設定
        ...
        test.setOrderSkuWashName(new ArrayList<FukukitaruOrderSkuModel>() {
            ...
            {
                add(createFKOrderSkuModel(BigInteger.valueOf(1)));
            }
        });// フクキタル発注:洗濯ネーム(1)
        test.setOrderSkuAttentionName(new ArrayList<FukukitaruOrderSkuModel>() {
            ...
            {
                add(createFKOrderSkuModel(BigInteger.valueOf(2)));
            }
        });// フクキタル発注:アテンションネーム(2)
        test.setOrderSkuWashAuxiliary(new ArrayList<FukukitaruOrderSkuModel>() {
            ...
            {
                add(createFKOrderSkuModel(BigInteger.valueOf(3)));
            }
        });// フクキタル発注:洗濯同封副資材(3)
        test.setOrderSkuBottomBill(new ArrayList<FukukitaruOrderSkuModel>() {
            ...
            {
                add(createFKOrderSkuModel(BigInteger.valueOf(4)));
            }
        });// フクキタル発注:下札(4)
        test.setOrderSkuAttentionTag(new ArrayList<FukukitaruOrderSkuModel>() {
            ...
            {
                add(createFKOrderSkuModel(BigInteger.valueOf(5)));
            }
        });// フクキタル発注:アテンションタグ(5)
        test.setOrderSkuBottomBillAttention(new ArrayList<FukukitaruOrderSkuModel>() {
            ...
            {
                add(createFKOrderSkuModel(BigInteger.valueOf(6)));
            }
        });// フクキタル発注:アテンション下札(6)
        test.setOrderSkuBottomBillNergyMerit(new ArrayList<FukukitaruOrderSkuModel>() {
            ...
            {
                add(createFKOrderSkuModel(BigInteger.valueOf(7)));
            }
        ...
        test.setOrderSkuBottomBillAuxiliaryMaterial(new ArrayList<FukukitaruOrderSkuModel>() {
            ...
            {
                add(createFKOrderSkuModel(BigInteger.valueOf(8)));
            }
        });// フクキタル発注:下札同封副資材(8)
        //*************************************************************/
        // フクキタル品番情報
        ...
        final MvcResult actual = mockServer
                .perform(MockMvcRequestBuilders.put("/api/v1/fukukitaru/orders/1/confirm")
    ...
    private FukukitaruOrderSkuModel createFKOrderSkuModel(BigInteger id) {
