...
@Transactional
public class ExtendedTItemRepositoryTest {
...
//    private ExtendedTItemRepository exTItemRepo;
    ...
    public void test_findByMisRepId_inDB() {
        try {
            //初期値：DBデータファイルパス
            final String initFilePath = new RepositoryTest().getFileAbsolutePath("/ExItemRepo");
...
//            final List<BigInteger> expected = Arrays.asList(BigInteger.valueOf(1), BigInteger.valueOf(2));
            ...
            //実値(sql実行)
//            Page<ExtendedTItemEntity> actual = exTItemRepo.findByMisRepId(BigInteger.valueOf(1),
//                    PageRequest.of(0, Integer.MAX_VALUE, Sort.by(Order.asc("part_no"))));
//
//            //テスト実行
//            //データの数の確認
...
//            }
    ...
    public void test_findByMisRepId_inDB_del() {
        try {
            ...
            //実値(sql実行)
//            Page<ExtendedTItemEntity> actual = exTItemRepo.findByMisRepId(BigInteger.valueOf(4), null);
//
//            //テスト実行
//            assertThat(actual.getContent().size()).isEqualTo(0);
    ...
    public void test_findByMisRepId_notInDB() {
        try {
            ...
            //実値(sql実行)
//            Page<ExtendedTItemEntity> actual = exTItemRepo.findByMisRepId(BigInteger.valueOf(45), null);
//
//            //テスト実行
//            assertThat(actual.getContent().size()).isEqualTo(0);
