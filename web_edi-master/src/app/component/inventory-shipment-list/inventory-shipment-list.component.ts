...
import { StorageKey, InstructorSystemType, Const } from 'src/app/const/const';
...
import { InventoryShipmentHttpService } from 'src/app/service/inventory-shipment-http.service';
...
@Component({
  selector: 'app-inventory-shipment-list',
  templateUrl: './inventory-shipment-list.component.html',
  styleUrls: ['./inventory-shipment-list.component.scss'],
  providers: [InventoryShipmentListStoreService]
})
export class InventoryShipmentListComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    private inventoryShipmentHttpService: InventoryShipmentHttpService,
   ...
   * @returns 検索フォーム初期値
   */
  private generateInitialFormValue(session: Session, stores: JunpcTnpmst[]): InventoryShipmentSearchCondition {
    // storageから前回入力した検索条件を取得する。
    const storageCondition: InventoryShipmentSearchCondition = JSON.parse(
      this.localStorageService.getSaveLocalStorage(session, StorageKey.INVESTORY_SHIPMENT_LIST_SEARCH_CONDITIONS));
    const formCondition = storageCondition == null ? new InventoryShipmentSearchCondition() : storageCondition;
    ...
    // 指示元の初期値は先頭
    searchCondition.instructorSystem = searchCondition.instructorSystem === undefined ?
      InstructorSystemType.JADORE : searchCondition.instructorSystem;
   ...
   */
  onClickSearchBtn(searchForm: InventoryShipmentSearchCondition): void {
    this.noCheck = true;
    let loadingToken = null;
    this.loadingService.start().pipe(
      tap(token => loadingToken = token),
   ...
   * @returns 検索結果
   */
  private search(searchFormValue: InventoryShipmentSearchCondition): Observable<InventoryShipmentSearchResult[]> {
    return this.inventoryShipmentHttpService.search(searchFormValue).pipe(
   ...
   * @param results 検索結果
   */
  private setSearchResultMessage(results: InventoryShipmentSearchResult[]): void {
    if (results.length === 0) {
   ...
   */
  private saveSearchConditions(session: Session, searchCondition: InventoryShipmentSearchCondition): void {
    this.localStorageService.createLocalStorage(session, StorageKey.INVESTORY_SHIPMENT_LIST_SEARCH_CONDITIONS, searchCondition);
  }
  ...
  onScrollToBottom(): void {
    ...
    this.search({ pageToken: nextPageToken } as InventoryShipmentSearchCondition).subscribe();
  }
  ...
  onLgSubmit(): void {
    ...
    this.loadingService.start().pipe(
      ...
      flatMap(formValue => this.inventoryShipmentHttpService.confirm(formValue)),
  ...
  private extractDivisionName = (result: GenericList<JunpcCodmst>): string =>
    this.divisionName = result == null || this.listUtils.isEmpty(result.items) ? '' : result.items[0].item1
