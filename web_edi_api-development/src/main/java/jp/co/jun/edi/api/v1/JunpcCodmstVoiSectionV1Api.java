...
import jp.co.jun.edi.service.JunpcCodmstVoiSectionListService;
import jp.co.jun.edi.type.MCodmstTblIdType;

/**
 * 発注生産システムのコードマスタからVoi区分情報を取得するAPI.
 */
@RestController
@RequestMapping("/api/v1/junpc/codmst/voiSections")
public class JunpcCodmstVoiSectionV1Api extends GenericJunpcCodmstV1Api {
    @Autowired
    private JunpcCodmstVoiSectionListService listService;

    /**
     * 発注生産システムのコードマスタからVoi区分情報をリストで取得する.
     *
    ...
    protected MCodmstTblIdType getTblId() {
        return MCodmstTblIdType.VOI_SECTION;
    }

    @Override
    protected JunpcCodmstVoiSectionListService getListService() {
        return listService;
    }
}
