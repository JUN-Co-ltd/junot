 ...
 */
public interface TPurchasesVoucherRepository extends JpaRepository<TPurchasesVoucherEntity, BigInteger> {
     ...
     */
    @Query("SELECT t FROM TPurchasesVoucherEntity t"
     ...
     */
    @Modifying
    @Query(value = "UPDATE t_purchases_voucher t"
            + " SET t.status = :status, "
            + " t.updated_at = now(), "
